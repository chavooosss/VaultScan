...
const SOURCE_LABELS = { paste: 'Kod', file: 'Dosya', github: 'GitHub' };
...
async function loadHistory() {
  ...
  try {
    ...
    if (data.items.length === 0) {
      list.innerHTML = '<p class="muted">Henüz hiç analiz yapmadın.</p>';
      return;
    }
    list.innerHTML = data.items.map(item => `
      <div class="history-item" data-id="${item.id}">
        ...
        <div class="history-item-actions">
          ...
          <button class="btn-copy key-remove-btn history-delete-btn" data-id="${item.id}">Sil</button>
        </div>
      </div>
    `).join('');
    ...
    list.innerHTML = `<p class="error-text">Hata: ${escapeHtml(e.message)}</p>`;
  }
}
...
async function deleteHistoryItem(id) {
  ...
  try {
    await userInfoReady;
    const resp = await fetch(`/api/history/${id}`, { method: 'DELETE', headers: { 'X-CSRF-Token': csrfToken } });
    const data = await resp.json();
    if (data.error) { alert(data.error); return; }
    loadHistory();
  } catch (e) {
